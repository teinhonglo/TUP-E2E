...
if $use_ground_truth; then
    dump_dir=dump
else
    dump_dir=dump_gt$use_ground_truth
fi
...
if [ ${stage} -le 0 ]; then
    for rtask in ${recog_set}; do
        ...
        feat_recog_dir=$dump_dir/${rtask}/delta${do_delta}
        ...
        data2json.sh --use-ground-truth $use_ground_truth --feat $dump_dir/$rtask/deltafalse/feats.scp \
                     --nlsyms $nlsyms data/${rtask} ${dict} > $dump_dir/$rtask/delta${do_delta}/data.json
    done
fi
...
if [ ${stage} -le 1 ]; then
    ...
    for rtask in ${recog_set}; do    
        ...
        # split data
        splitjson.py --parts ${nj} $dump_dir/${rtask}/delta${do_delta}/data.json
        ...
        ${decode_cmd} JOB=1:${nj} ${expdir}/${decode_dir}/log/decode.JOB.log \
            ...
            --nbest 5 \
            --use-ground-truth $use_ground_truth
            #--lm-weight ${lm_weight} \
...
if [ $stage -le 2 ]; then
    for rtask in ${recog_set}; do
        decode_dir=decode_${rtask}_beam${beam_size}_e${recog_model}_p${penalty}_len${minlenratio}-${maxlenratio}_ctcw${ctc_weight}_L1
        local/capt/run_CAPT.sh --expdir ${expdir}/${decode_dir}
    done
fi
