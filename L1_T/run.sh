...
if [ ${stage} -le 1 ]; then
    ...
    fbankdir=fbank
    # speed-perturbed
    #utils/data/perturb_data_dir_speed_3way.sh data/L1_T_train data/L1_T_train_sp
    # remove long short
    #remove_longshortdata.sh --maxframes 3000 --maxchars 400 data/L1_T_train_sp data/L1_T_train_sp_re
...
dict=data/lang_1char/L1_T_train_sp_units.txt
...
if [ -z ${tag} ]; then
    expdir=exp/train_lsm_${backend}_${etype}_e${elayers}_subsample${subsample}_unit${eunits}_proj${eprojs}_d${dlayers}_unit${dunits}_${atype}_aconvc${aconv_chans}_aconvf${aconv_filts}_mtlalpha${mtlalpha}_${opt}_sampprob${samp_prob}_bs${batchsize}_mli${maxlen_in}_mlo${maxlen_out}
    if ${do_delta}; then
